...
@poll_required
async def open_due_date_datepicker(
    _: Session, context: CallbackContext, poll: Poll
) -> None:
    ...
    await context.query.message.edit_reply_markup(reply_markup=keyboard)


@poll_required
async def show_styling_menu(
    context: CallbackContext, poll: Poll
) -> None:
    """Show the menu for sorting settings."""
    await context.query.message.edit_text(
        get_poll_text(get_session(), context.poll),
        parse_mode="markdown",
...
def remove_option(
    ...
    """Remove the option."""
    get_session().query(Option).filter(Option.id == context.action).delete()

    if poll.is_priority():
        reorder_votes_after_option_delete(get_session(), poll)
